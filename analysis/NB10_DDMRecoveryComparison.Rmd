...
Have seen both failied recovery, successful recovery and split posterior with with 50 particles.
...
Posterior surface is split between two models but at least one of them is the correct one.
...
**Sum of trial likelihoods from the state-space approach does not seem viable to estimate parameters**
**What is the intuition behind the likelihood computation from the RT histograms conditioned on choice?**
...
# optim(c(.01, .01), get_task_nll, data=stim1, par_names = c("d", "sigma"), model_name="model1", control = list(maxit=25), control=list(trace=TRUE))

## use `capture.output` to get raw output
out <- capture.output(df <- optim(c(.01, .01), get_task_nll, data=stim1, 
                                  par_names = c("d", "sigma"), model_name="model1", control = list(maxit=25, trace=TRUE)))
