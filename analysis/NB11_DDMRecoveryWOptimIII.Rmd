...
To download sim output
`docker run --rm -it -v ~/.aws:/root/.aws -v $(pwd):/sim3 amazon/aws-cli s3 sync  s3://described-vs-experienced/ddModels/cluster_scripts/optim_out/sim3 /sim3`

```{r}
data_suffix = "sim_single_sub_data46"
data = read.csv(paste0(helpers_path, 'ddModels/cluster_scripts/test_data/', data_suffix, '.csv'))
```
...
```{r}
model_name = "model1c"
data_name = "sim_single_sub_data66"
optim_out_path = paste0(cpueaters_paths, 'ddModels/cluster_scripts/optim_out/sim3/')

tmp = ddm_par_recovery_report(model_ = model_name, data_ = data_name, optim_out_path_=optim_out_path, diff_pct_plots_ = TRUE)

tmp = get_true_pars(data_ = data_name, true_pars_path_ = true_pars_path)
...
For the same 36 datasets fix delta to 1. Optimize over d and sigma. Then fix d and sigma and optimize over delta.

**Example1**

```{r}
data_suffix = "sim_single_sub_data63"
data = read.csv(paste0(helpers_path, 'ddModels/cluster_scripts/test_data/', data_suffix, '.csv'))
```
...
second_start_vals = c(2.624)
second_par_names = c("delta")
...
```{r}
data_suffix = "sim_single_sub_data64"
data = read.csv(paste0(helpers_path, 'ddModels/cluster_scripts/test_data/', data_suffix, '.csv'))
```
...
```{r}
data_suffix = "sim_single_sub_data65"
data = read.csv(paste0(helpers_path, 'ddModels/cluster_scripts/test_data/', data_suffix, '.csv'))
```
...
How to combine and save the results in ddm_Roptim.R?

```{r}
first_optim_out$iterations_df %>%
  mutate(Param3 = 1) %>%
