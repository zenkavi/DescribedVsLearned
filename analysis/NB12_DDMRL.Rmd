...
optim_out_path = paste0(cpueaters_paths, 'ddrlModels/cluster_scripts/optim_out/sim1/')
true_pars_path = paste0(helpers_path, 'ddrlModels/cluster_scripts/test_data/')
...
out_iters = out_iters %>%
  rename(d = Param1, sigma = Param2, alpha = Param3, delta = Param4, nll = Result)
```

Why are there only five iterations for each kernel?

```{r}
table(out_iters$Iteration)
```

```{r}
par_recovery_report(model_ = model_name, data_ = paste0(data_prefix, 1), optim_out_path_ = optim_out_path, diff_pct_plots_ = TRUE, true_pars_path_=true_pars_path, param_dict_=param_dict)
```