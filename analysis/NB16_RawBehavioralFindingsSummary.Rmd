...
clean_beh_data %>%
  ...
  # facet_grid(probFractalDraw ~ session)
  facet_wrap(~session)
```

How is the relevance of attributes changing across the task for each subject? Pretty erratically.
There is a lot going on on the reward screen
...
is the fractal rewarded in the last trial
is the fractal relevant in this trial
is the better fractal bundle chosen in this trial
increase lag in the same plot to see if the effect decreases

```{r}
clean_beh_data %>%
  select(subnum, trialNum, choiceLeft, reward, fractalDraw, probFractalDraw, choseBetterFractalBundle) %>%
  group_by(subnum) %>% # so the last choice for one subject does not bleed into the first choice of the next subject
  mutate(
         lastFractalDraw = lag(fractalDraw),
  ...
  group_by(lastFractalDraw, lastTrialRewarded, probFractalDraw) %>%
  summarise(.groups='keep',
            prop_choseBetterFractalBundle = mean(choseBetterFractalBundle, na.rm=T),
            sem_choseBetterFractalBundle = sd(choseBetterFractalBundle, na.rm=T)/sqrt(n())) %>%
  mutate(lastRewardAttribute = ifelse(lastFractalDraw == 1, "fractal", "lottery")) %>%  
  ggplot(aes(probFractalDraw, prop_choseBetterFractalBundle, color=lastRewardAttribute))+
  geom_point(position = position_dodge(width=.5))+
  geom_errorbar(aes(ymin=prop_choseBetterFractalBundle-sem_choseBetterFractalBundle, ymax=prop_choseBetterFractalBundle+sem_choseBetterFractalBundle), width=.2, position=position_dodge(width=.5))+
  geom_hline(aes(yintercept=.5), color="gray")+
  facet_grid(lastTrialRewarded~.)+
  theme(legend.position = "bottom")+
  labs(x= "p(Fractal)", y = "Proportion of better fractal bundle choice", color="Last rewarded attribute")+
  scale_color_manual(values=cbbPalette[1:2],
                     breaks = c("fractal", "lottery"),
                     labels = c("Fractal", "Lottery"))
...
We don't have losses in this experiment. Only gains.
Could we quantify risk attitude in any way? What would be risky in this paradigm? Choosing based on fractals before you've seen enough to form a good representation of their value. Choosing the bundle with the lottery that has the same or larger EV.
