...
Does it depend on the amount or probability of the varying lottery? No. Not for anyone (or any probFractalDraw level).
...
# clean_beh_data %>%
#   select(subnum, reactionTime, lotteryValue, lotteryProb, probFractalDraw) %>%
#   gather(key, value, -subnum, -reactionTime, -probFractalDraw) %>%
#   ggplot(aes(value, reactionTime))+
#   geom_line(aes(group=subnum),stat='smooth',formula = 'y~x', method = "lm",se=FALSE, alpha=.1, size=1)+
#   geom_smooth(formula = 'y~x', method = "lm")+
#   facet_grid(probFractalDraw~key, scales="free_x")
```
...
clean_beh_data %>%
  mutate(logRt = log(reactionTime),
         ...
         conflictTrial = ifelse(EVLeft<EVRight & fractalLeftProb>fractalRightProb, "conflict", ifelse(EVLeft>EVRight & fractalLeftProb<fractalRightProb, "conflict", "no conflict"))) %>%
...
```{r}
clean_beh_data %>%
  mutate(fractal_diff = abs(fractalLeftProb - fractalRightProb),
         lottery_ev_diff = round(abs((referenceProb*referenceValue) - (lotteryProb*lotteryValue)),3),
...
clean_beh_data %>%
  mutate(fractal_diff = abs(fractalLeftProb - fractalRightProb),
         lottery_ev_diff = round(abs((referenceProb*referenceValue) - (lotteryProb*lotteryValue)),3),
         lottery_ev_diff = ifelse(lottery_ev_diff ==0, "no EV diff", ifelse(lottery_ev_diff == .2, "small EV diff", ifelse(lottery_ev_diff == .4, "large EV diff", NA))),
         lottery_ev_diff = factor(lottery_ev_diff, levels=c("no EV diff", "small EV diff", "large EV diff")),
         logRt = log(reactionTime)) %>%  
  ggplot(aes(fractal_diff, logRt)) + 
  geom_line(aes(group=subnum),stat='smooth',formula = 'y~x', method = "lm",se=FALSE, alpha=.1, size=1)+
  geom_smooth(formula = 'y~x', method = "lm")+
...
Are they faster when they choose left vs right? Since the learned attribute options are both fractals the stimulus difference lies in the lotteries. The bundle on the **right** has the fixed and the one on the right has the varying lottery. So right choices might be slower since they require more processing.
...
clean_beh_data %>%
  ...
  summarise(.groups="keep",
            ...
            sem_logRt = sd(log_Rt)/sqrt(n())) %>%
  mutate(probFractalDraw = as.factor(probFractalDraw),
         choiceLeft = ifelse(choiceLeft == 0, "right (varying Lottery)", "left (reference Lottery)")) %>%
...
```{r}
clean_beh_data %>%
  filter((lotteryProb*lotteryValue)-(referenceProb*referenceValue) != 0 ) %>%
  mutate(leftLotteryBetter = (lotteryProb*lotteryValue)-(referenceProb*referenceValue)>0,
  ...
  summarise(.groups="keep",
            ...
            sem_logRt = sd(log_Rt)/sqrt(n())) %>%
  mutate(probFractalDraw = as.factor(probFractalDraw),
         choseBetterLotteryBundle = as.factor(choseBetterLotteryBundle)) %>%
...
This effect is driven more by no conflict trials.
...
```{r}
clean_beh_data %>%
  mutate(leftFractalBetter = (fractalLeftProb)-(fractalRightProb)>0,
  ...
  summarise(.groups="keep",
            ...
            sem_logRt = sd(log_Rt)/sqrt(n())) %>%
  mutate(probFractalDraw = as.factor(probFractalDraw),
         choseBetterFractalBundle = as.factor(choseBetterFractalBundle)) %>%
  ggplot(aes(probFractalDraw, mean_logRt, color=choseBetterFractalBundle))+
  geom_point()+
  geom_errorbar(aes(ymin = mean_logRt - sem_logRt, ymax = mean_logRt + sem_logRt), width=.2)+
  theme(legend.position = "bottom")
```
Note: This is not the exact opposite of the plot based on choosing the better lottery because while in each trial there is a better fractal in some trials the lottery EVs are the same. If there isn't any distortion of EVs and equal EVs lead to indifference subjects can still choose based on the fractal value difference. Then choosing the bundle with the better fractal even in trials where the fractals are less relevant for the reward would still be reasonable.
...
clean_beh_data %>%
  mutate(leftFractalBetter = (fractalLeftProb)-(fractalRightProb)>0,
         ...
         EVRight = lotteryValue * lotteryProb,
         conflictTrial = ifelse(EVLeft<EVRight & fractalLeftProb>fractalRightProb, "conflict", ifelse(EVLeft>EVRight & fractalLeftProb<fractalRightProb, "conflict", "no conflict"))) %>%
  # filter(EVRight != EVLeft) %>% #excluding these makes it the same as the plot in the previous section
  group_by(probFractalDraw, choseBetterFractalBundle, conflictTrial) %>%
  summarise(.groups="keep",
            mean_logRt = mean(log_Rt),
            sem_logRt = sd(log_Rt)/sqrt(n())) %>%
  mutate(probFractalDraw = as.factor(probFractalDraw),
...
```{r}
clean_beh_data %>%
  mutate(EVLeft = referenceProb * referenceValue,
         EVRight = lotteryValue * lotteryProb,
         conflictTrial = ifelse(EVLeft<EVRight & fractalLeftProb>fractalRightProb, "conflict", ifelse(EVLeft>EVRight & fractalLeftProb<fractalRightProb, "conflict", "no conflict"))) %>%
...
When the lotteries don't have the same EV do subjects choose the bundle with the better lottery? 

No. Proportions especially for probFractalDraw <= .5 suggests terrible performance for both conflict and no conflict trials. If performance was good we should see a flip from high to low in conflict trials as probFractalDraw changes and a constant above chance level for no conflict trials.
...
clean_beh_data %>%
  ...
  mutate(probFractalDraw = as.factor(probFractalDraw),
         leftLotteryBetter = EVLeft > EVRight,
         choseBetterLotteryBundle = ifelse(leftLotteryBetter & choiceLeft == 1, 1, ifelse(!leftLotteryBetter & choiceLeft == 0, 1, 0))) %>%
...
Do they get better with time? Not really.
...
In conflict trials subjects choose the bundle with the better fractal (as opposed to the one with the better lottery) too frequently when the reward depends more on the lotteries.  

prbFractalDraw =< .5 for no conflict trials also suggests bad performance. Subjects should choose the bundle with the better fractal, which is also the bundle with the better lottery more often than chance.  
...
clean_beh_data %>%
  ...
  mutate(probFractalDraw = as.factor(probFractalDraw),
         leftFractalBetter = fractalLeftProb > fractalRightProb,
         choseBetterFractalBundle = ifelse(leftFractalBetter & choiceLeft == 1, 1, ifelse(!leftFractalBetter & choiceLeft == 0, 1, 0))) %>%
  group_by(probFractalDraw, subnum, conflictTrial) %>%
  summarise(.groups="keep",
...
These didn't suggest terrible performance in NB3 when I used the QValues.

Check the same plots with Qvalues and 
Check correlation between QValues with true values (fractalProbs)

```{r}
source(paste0(helpers_path, 'add_inferred_pars.R'))
source(paste0(helpers_path, 'rlModels/fit_rl_hierarchical.R'))
clean_beh_data_ip = add_inferred_pars(clean_beh_data, par_ests)
```

This one suggests performance is reasonable with the expected flip for conflict trials and persistent highs for no conflict
...
clean_beh_data_ip %>%
  ...
  group_by(probFractalDraw, subnum, conflictTrial) %>%
  summarise(.groups="keep",
            prop_choseBetterLotteryBundle = mean(choseBetterLottery)) %>%
  ggplot(aes(probFractalDraw, prop_choseBetterLotteryBundle))+
  geom_boxplot()+
  geom_hline(aes(yintercept=.5), color="grey")+
  facet_grid(conflictTrial~.)
...
```
There is some discrepancies between true fractal and q values but the larger difference is in how I've defined conflict trials in this nb.
...
clean_beh_data %>%
  mutate(EVLeft = referenceProb * referenceValue,
         EVRight = lotteryValue * lotteryProb,
         conflictTrial = ifelse(EVLeft<EVRight & fractalLeftProb>fractalRightProb, "conflict", ifelse(EVLeft>EVRight & fractalLeftProb<fractalRightProb, "conflict", "no conflict")),
         leftLotteryBetter = (lotteryProb*lotteryValue)-(referenceProb*referenceValue)>0,
         leftFractalBetter = (fractalLeftProb)-(fractalRightProb)>0) %>%
  select(subnum, trialNum, session, conflictTrial, leftFractalBetter, leftLotteryBetter) %>%
  left_join(clean_beh_data_ip %>%
              select(subnum, trialNum, session, conflictTrial, leftFractalBetter, leftLotteryBetter),
            by=c('subnum', 'trialNum', 'session')) %>%
  mutate(sameConflict = ifelse(conflictTrial.x == conflictTrial.y, 1, 0),
         sameLotteryBetter = ifelse(leftLotteryBetter.x == leftLotteryBetter.y, 1, 0),
         sameFractalBetter = ifelse(leftFractalBetter.x == leftFractalBetter.y, 1, 0)) %>%
  summarise(sumSameConflict = sum(sameConflict),
            sumSameLottery = sum(sameLotteryBetter),
            sumSameFractal = sum(sameFractalBetter))
```
```{r}
clean_beh_data_ip %>%
  ggplot(aes(fractalLeftProb, leftQValue))+
...
```{r}
clean_beh_data_ip %>%
  ggplot(aes(fractalRightProb, rightQValue))+
  geom_point(color="gray")+
  geom_abline(aes(intercept=0, slope=1), color="black")+
  geom_smooth(method="lm", formula='y~x')
```

What is wrong with how I define conflict in this nb

LEFT has the varying lottery! Not right!
...
clean_beh_data %>%
  mutate(EVRight = referenceProb * referenceValue,
         ...
         leftLotteryBetter = (lotteryProb*lotteryValue)-(referenceProb*referenceValue)>0,
         leftFractalBetter = (fractalLeftProb)-(fractalRightProb)>0) %>%
  select(EVRight, EVLeft, fractalLeftProb, fractalRightProb, leftLotteryBetter, leftFractalBetter, conflictTrial)

```
Do the two definitions match better when you fix the EV sides? Yes.
...
clean_beh_data %>%
  ...
  summarise(sumSameConflict = sum(sameConflict),
            ...
            sumSameFractal = sum(sameFractalBetter))
```
Are the differences in conflict definitions only due to true versus q values?
Different conflict definitions are either differing q vs true values OR when the lotteries have the same value.
That is defined better in this NB I think. 
...
clean_beh_data %>%
  ...
  select(subnum, trialNum, session, conflictTrial, leftFractalBetter, leftLotteryBetter, lotteriesSame) %>%
  left_join(clean_beh_data_ip %>%
              select(subnum, trialNum, session, conflictTrial, leftFractalBetter, leftLotteryBetter),
            by=c('subnum', 'trialNum', 'session')) %>%
  select(-subnum, -trialNum, -session, -leftLotteryBetter.x, -leftLotteryBetter.y) %>%
  mutate(sameConflict = ifelse(conflictTrial.x == conflictTrial.y, 1, 0),
         sameFractalBetter = ifelse(leftFractalBetter.x == leftFractalBetter.y, 1, 0)) %>%
  filter(sameConflict == 0 & sameFractalBetter == 1)
```
...
clean_beh_data %>%
  ...
  mutate(sameConflict = ifelse(conflictTrial.x == conflictTrial.y, 1, 0),
         sameFractalBetter = ifelse(leftFractalBetter.x == leftFractalBetter.y, 1, 0)) %>%
  filter(sameConflict == 0 & sameFractalBetter == 0)
```
...
1. objective lottery, 
2.fractal and 
3.weighted bundle values

## Perseverance of choice 

(choosing the rewarded side)

## Correct weighting of relevant attribute
...
learning doesn't look strong looking at the slopes of these curves.
...
clean_beh_data %>%
  ...
  mutate(trialNum = 1:n()) %>%
  ungroup() %>%
  mutate(choseBundleWBetterFractal = ifelse(fractalRightProb>fractalLeftProb & choiceLeft == 0, 1, ifelse(fractalLeftProb>fractalRightProb & choiceLeft == 1, 1, 0)),
         EVLeft = referenceProb * referenceValue,
         EVRight = lotteryValue * lotteryProb,
         conflictTrial = ifelse(EVLeft<EVRight & fractalLeftProb>fractalRightProb, "conflict", ifelse(EVLeft>EVRight & fractalLeftProb<fractalRightProb, "conflict", "no conflict"))) %>%
