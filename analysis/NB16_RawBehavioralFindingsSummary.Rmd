...
### Logit with ambiguity aversion

Before you fit anything, what does the fractalDiff curve look like if you just multiply it with a constant? Can you mostly match the EVDiff curve to support the idea of a constant ambiguity aversion?
...
clean_beh_data %>%
  ...
  filter(term != "(Intercept)") %>%
  select(probFractalDraw, term, estimate, std.error) %>%
  mutate(estimate = ifelse(term == "fractalDiff", estimate*3.5, estimate))%>%
  mutate(term = ifelse(term == "EVDiff", "Lottery EV difference", ifelse(term == "fractalDiff", "Fractal probability difference", NA))) %>%
  ggplot(aes(probFractalDraw, estimate, col=term, group=term))+
      geom_point()+
      geom_line()+
...
But based on the size of the individual differences maybe I shouldn't be looking for a group level learning effect and instead split the sample into learners vs not learners.
...
ggsave(file=paste0(fig_out_path, 'choice_of_rewarded_fractal.jpg'), p, height = 5, width=10, units="in")
p
```
...
clean_beh_data %>%
  ...
  ungroup() %>%
  mutate(session = paste0("run ", session)) %>%
  # filter(subnum %in% c("01", "02", "03","04")) %>%
  filter(subnum %in% c("05", "06", "07","08")) %>% #08
  # filter(subnum %in% c("09", "10", "11","12")) %>% 
  # filter(subnum %in% c("13", "14", "15","16")) %>% 
  # filter(subnum %in% c("17", "18", "19","20")) %>% 
  # filter(subnum %in% c("22", "23", "24","25")) %>% 
  
  filter(as.numeric(as.character(probFractalDraw))>.5) %>%
  ggplot(aes(trialNum, choseMoreRewardedFractal, color=subnum))+
  geom_smooth(formula = 'y~x', method = "glm", method.args = list(family=binomial), se=FALSE, fullrange=TRUE, alpha=.25)+
  # geom_line(aes(color=subnum),stat='smooth',formula = 'y~x', method = "lm",se=FALSE, alpha=.5, size=1)+
  # geom_line(stat='smooth',formula = 'y~x', method = "lm",se=FALSE, alpha=.5, size=1)+
...
clean_beh_data %>%
  ...
  ungroup() %>%
  mutate(session = paste0("run ", session), 
         trialGroup = ifelse(trialNum < 21, "first", ifelse(trialNum < 41 & trialNum > 20, "second", "third"))) %>%
  # select(subnum, session, trialNum, trialGroup, choseMoreRewardedFractal)
  group_by(session, subnum, trialGroup) %>%
  summarise(.groups="keep", 
            mean_choseMoreRewardedFractal = mean(choseMoreRewardedFractal, na.rm=T),
            sem_choseMoreRewardedFractal = sd(choseMoreRewardedFractal, na.rm=T)/sqrt(n())) %>%
  ggplot(aes(trialGroup, mean_choseMoreRewardedFractal))+
  geom_boxplot()+
  geom_hline(aes(yintercept=.5), color="gray")+
  facet_grid(.~session)
```
