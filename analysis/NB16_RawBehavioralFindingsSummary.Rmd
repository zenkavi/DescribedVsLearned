...
Are they faster when they choose left vs right? Since the learned attribute options are both fractals the stimulus difference lies in the lotteries. The bundle with on the left has the fixed and the one on the right has the varying lottery. So right choices might be slower since they require more processing.

When lotteries matter more for the reward they are slightly slower when choosing the bundle with the varying lottery. This suggests some computing time for the EV of the varying lottery.  

When fractal matters more for the reward the side effect switches. Does this suggest a side bias for when reward depends more on fractals?
...
clean_beh_data %>%
  ...
  geom_errorbar(aes(ymin = mean_logRt - sem_logRt, ymax = mean_logRt + sem_logRt), width=.2, position=position_dodge(width=.25))+
  theme(legend.position = "bottom",
        legend.title = element_blank())
```

The differences between left and right choice speeds at different probFractalDraw levels are driven differently by conflict and no conflict trials.
