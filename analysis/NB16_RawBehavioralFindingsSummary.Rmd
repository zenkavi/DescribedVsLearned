...
tmp3 = clean_beh_data %>%
  filter(leftBundleValue != rightBundleValue) %>%
   mutate(session = paste0("run ", session)) %>%
  group_by(probFractalDraw, conflictTrial, session) %>%
  summarise(.groups="keep",
            prop_choseBetterOverallBundle = mean(choseBetterOverallBundle),
...
unique(tmp3$prop_choseBetterOverallBundle)
```


```{r}
tmp2 %>%
  left_join(tmp3, by=names(tmp2)[1:3]) %>%
  gather(prop_type, prop_value, -probFractalDraw, -conflictTrial, -session,-sem_choseBetterFractalBundle, -sem_choseBetterOverallBundle) %>%
  gather(sem_type, sem_value,  -probFractalDraw, -conflictTrial, -session, -prop_type, -prop_value) %>%
  ggplot(aes(probFractalDraw, prop_value, color=prop_type))+
  geom_point(alpha=.5)+
  geom_errorbar(aes(ymin = prop_value-sem_value,
...
tmp2 %>%
  ...
  filter(conflictTrial == "no conflict" & probFractalDraw == 0 & session == "run 2")
```

```{r}
clean_beh_data %>%
  select(-trialNum, -referenceValue, -referenceProb, -numRewardedTrials, -fractalDraw, -reactionTime, -responded, -reward, -totalReward, -logRt) %>%
  mutate(probFractalDraw = as.numeric(as.character(probFractalDraw))) %>%
  filter(conflictTrial == "conflict" & probFractalDraw == 0.9 & session == 2) %>%
  mutate(same = choseBetterFractalBundle == choseBetterOverallBundle) %>%
  arrange(same) %>%
  select(fractalLeftProb, fractalRightProb, EVLeft, EVRight, leftBundleValue, rightBundleValue)
```
