...
Are you more likely to choose the better fractal bundle as trialnum increases? No. No difference depending on conflict trial either.
...
Are you more likely to choose the better fractal bundle as trialnum increases for trials where the fractals matter more for the reward? Somewhat in earlier runs.
...
But based on the size of the individual differences (no longer shown in the plots above) maybe I shouldn't be looking for a group level learning effect and instead split the sample into learners vs not learners. No, the within subject slopes are not consistent across runs.  
...
clean_beh_data %>%
  ...
  summarise(.groups="keep",
    ...
    gather(prop_type, prop_value, -probFractalDraw, -conflictTrial, -session,-sem_choseBetterLotteryBundle ,-sem_choseBetterFractalBundle, -sem_choseBetterOverallBundle) %>%
  gather(sem_type, sem_value,  -probFractalDraw, -conflictTrial, -session, -prop_type, -prop_value) %>%
  filter(prop_type == "prop_choseBetterFractalBundle") %>%
  ggplot(aes(probFractalDraw, prop_value, color=prop_type))+
  geom_point(alpha=.5)+
  geom_errorbar(aes(ymin = prop_value-sem_value,
                    ymax = prop_value+sem_value), width=.2)+
...
So far, I have not found a clear signal for learning in raw behavioral data.  

One suggestion from lab meeting was to compare predicted probability versus true proportion of choice for different levels of attribute relevance (?). I've actually done this before by sampling from subject posteriors n=100 times, generating choice data and comparing it true subject data. Overall we found that most subjects' choice at various levels fit in the middle of the posterior predictive distribution. This suggests that the choice behavior in the task can be recapitulated by RL models to a decent degree.  

**Find a more succinct way of visualizing and summarizing posterior predictive data/model fit**
...
Because I don't have a clear signal for learning in raw behavior I'm not sure how to test these ideas in behavior alone. Instead, I might expand on the analyses I did [here](https://zenkavi.github.io/DescribedVsLearned_beh/outputs/NB4_indiv_differences_in_rpe_models.html) fitting RL models with different update rules and checking whether they explain data better.
...
- Assumes a lot in functional forms especially since in neither case there is an explicity certainty equivalent  
- Description-based choice is not strictly description only since it's outcome is played out in each trial  
...
- Asymmetric distortion of attribute relevance: Lottery slopes decline linearly but fractal slopes might have a more step-like shape.  
...
- Did not find a clean signal of learning from raw behavioral measures.  
