...
tmp = clean_beh_data %>%
  group_by(subnum) %>%
  mutate(diff_level = ifelse(qv_diff < quantile(qv_diff, probs=c(.33))[[1]], "small",
  ...
  summarise(.groups = "keep",
            mean_logRt = mean(logRt),
            sem_logRt = sd(logRt)/sqrt(n())) %>%
  mutate(diff_type = "Learned")

p = clean_beh_data %>%
  group_by(subnum) %>%
  mutate(diff_level = ifelse(fractal_diff < quantile(fractal_diff, probs=c(.33))[[1]], "small",
  ...
  summarise(.groups = "keep",
            mean_logRt = mean(logRt),
            sem_logRt = sd(logRt)/sqrt(n())) %>%
  mutate(diff_type="True") %>%
  rbind(tmp) %>%
  ggplot(aes(probFractalDraw, mean_logRt,color=diff_level, alpha=diff_type))+
  geom_point(position=position_dodge(width=.5), size=3)+
  geom_errorbar(aes(ymin = mean_logRt - sem_logRt, ymax = mean_logRt + sem_logRt), width=0,position=position_dodge(width=.5), size=2)+
  labs(color="Difference Level", y="Mean Log RT", x="p(Fractal)", alpha = "Value type")+
  scale_color_manual(values = c(cbbPalette[3], cbbPalette[5:6]))+
   theme(legend.position = c(.34,.2),
        legend.background = element_blank(),
        panel.grid = element_blank(),
...
p = clean_beh_data %>%
  ...
  mutate(choseBetterFractalBundle = ifelse(choseBetterFractalBundle ==1, "TRUE", "FALSE"),
         basedOn = "Fractal") %>%
  rename(betterChoice = choseBetterFractalBundle)) %>%
