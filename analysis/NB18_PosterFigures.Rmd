...
clean_beh_data %>%
  ...
  mutate(choseBetterFractalBundle = ifelse(choseBetterFractalBundle ==1, "TRUE", "FALSE"),
         basedOn = "Fractal") %>%
  rename(betterChoice = choseBetterFractalBundle)) %>%
  ggplot(aes(probFractalDraw, mean_logRt, shape=betterChoice, color=basedOn))+
  geom_point( position = position_dodge(width = .5), size=3)+
  ...
  labs(x="p(Fractal)", y = "Mean log RT", color="", shape="Better Choice")+
  scale_color_manual(values = c(cbbPalette[3], cbbPalette[5], cbbPalette[6], cbbPalette[7]))+
  scale_y_continuous(breaks = c(-.3, 0, .3), limits=c(-.36, .36))
