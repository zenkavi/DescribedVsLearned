...
ggsave(file=paste0(fig_out_path, 'poster_rt_by_value_diff.jpg'), p, height = 15, width=8, units="in")
p
```
...
out %>%
  ...
  ggplot(aes(step, rdv))+
  geom_line(group=1) +
  geom_hline(aes(yintercept=max(out$rdv)))+
  geom_hline(aes(yintercept=(-1)*max(out$rdv)))+
  geom_hline(aes(yintercept=0), linetype="dashed")+
  # theme_classic()+
