...
mean_logLiks = data.frame(subnum = NA, mean_logLik = NA, model_name = NA, numpar = NA, symm = NA, shape = NA, rpe = NA)
    ...
    for(k in 1:length(shape_names)){
      for(l in 1:length(rpe_names)){
        ...
        tmp = par_ests %>%
          filter(par == "alpha") %>%
          group_by(subnum) %>%
          summarise(mean_logLik = mean(logLik)) %>%
        ...
        mean_logLiks = rbind(mean_logLiks, tmp)
...
mean_logLiks = mean_logLiks %>% drop_na()
