...
```{r}
plot_opts = list(scale_alpha_manual(values=c(.6, 1), name="Distortion shape",
...
```
Note that this difference isn't as large as the plot above suggests. The expected log prob densities for cross validation show a similar trend but are indistinguishable across the models.
