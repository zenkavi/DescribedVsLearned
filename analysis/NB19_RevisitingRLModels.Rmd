...
model_names = c("oneParamDoubleSymmLinearProbDistortion_rpeBoth")
...
elpds = data.frame(Estimate = NA, SE = NA, model_name = NA, numpar=  NA, symm = NA, shape = NA, rpe=NA, norm=NA)
sampled_logLiks = data.frame(subnum = NA, logLik = NA, model_name=NA)
loaded_sampled_logLiks = FALSE
...
for(i in 1:length(model_names)){
  ...
  cur_row = data.frame(loo::loo(logLik)$estimates)[1,]
  cur_info = model_info %>% filter(model_name == cur_model)
  cur_row = cbind(cur_row, cur_info)  
    
  elpds = rbind(elpds, cur_row)
  
  rbind(elpds)
  
  if(file.exists(paste0(helpers_path, 'rlModels/sampled_logLiks.RDS')) & !loaded_sampled_logLiks){
    readRDS(paste0(helpers_path, 'rlModels/sampled_logLiks.RDS'))
    loaded_sampled_logLiks = TRUE
  } else{
   tmp = par_ests %>%
    filter(par == "alpha") %>%
    group_by(subnum) %>% 
...
```{r}
elpds %>%
  mutate(model_name = reorder(model_name, Estimate)) %>%
...
elpds %>%
  ...
  select(model_name, elpd_rank, mean_logLik_rank) %>%
  mutate(rank_diff = elpd_rank - mean_logLik_rank)
```
...
sampled_logLiks %>%
  group_by(subnum) %>%
  summarise(minLogLik = min(logLik))
```
