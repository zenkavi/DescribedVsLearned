...
clean_beh_data_nonorm = par_ests %>%
  group_by(subnum, par) %>%
  summarise(est = mean(value), .groups='keep') %>%
  spread(par, est) %>%
  left_join(clean_beh_data, by='subnum')
...
## Add Q values of fractals to each trial
clean_beh_data_nonorm = clean_beh_data_nonorm %>%
  group_by(subnum) %>%
  do(get_qvals(., model_name="rpeBoth")) %>%
  ungroup()
...
thetas %>%
  ...
  geom_vline(aes(xintercept = 1), color="#F8766D")+
  geom_vline(aes(xintercept = .5), color="#00BFC4")+
  facet_wrap(~subnum)+
  theme(panel.grid = element_blank(),
...
What do the QValues look like?

```{r}
clean_beh_data_norm %>%
  select(leftQValue, subnum) %>%
  mutate(model="SymmNorm") %>%
  group_by(subnum) %>%
  mutate(trialNum = 1:n()) %>%
  rbind(clean_beh_data_nonorm %>%
          select(leftQValue, subnum) %>%
          mutate(model="Asymm") %>%
          group_by(subnum) %>%
          mutate(trialNum = 1:n())) %>%
  group_by(subnum, trialNum) %>%
  spread(model, leftQValue) %>%
  ggplot(aes(Asymm, SymmNorm))+
  geom_point()+
  ...
  labs(title="left Q Value")

```

```{r}
clean_beh_data_norm %>%
  select(rightQValue, subnum) %>%
  mutate(model="SymmNorm") %>%
  group_by(subnum) %>%
  mutate(trialNum = 1:n()) %>%
  rbind(clean_beh_data_nonorm %>%
  ...
  theme(panel.grid = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank())+
  labs(title="right Q Value")
```

What do the distorted EV and QV diffs look like?

```{r}
clean_beh_data_norm %>%
  select(distorted_ev_diff, subnum) %>%
  mutate(model="SymmNorm") %>%
  group_by(subnum) %>%
  mutate(trialNum = 1:n()) %>%
  rbind(clean_beh_data_nonorm %>%
          ...
          mutate(trialNum = 1:n())) %>%
  group_by(subnum, trialNum) %>%
  spread(model, distorted_ev_diff) %>%
  ggplot(aes(Asymm, SymmNorm))+
  geom_point()+
  geom_abline(aes(intercept=0, slope=1))+
  facet_wrap(~subnum)+
  theme(panel.grid = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank())+
  labs(title="distorted EV Diff")
```
```{r}
clean_beh_data_norm %>%
  select(distorted_qv_diff, subnum) %>%
  mutate(model="SymmNorm") %>%
  group_by(subnum) %>%
  mutate(trialNum = 1:n()) %>%
  rbind(clean_beh_data_nonorm %>%
          ...
          mutate(trialNum = 1:n())) %>%
  group_by(subnum, trialNum) %>%
  spread(model, distorted_qv_diff) %>%
  ggplot(aes(Asymm, SymmNorm))+
  geom_point()+
  geom_abline(aes(intercept=0, slope=1))+
  facet_wrap(~subnum)+
  theme(panel.grid = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank())+
  labs(title="distorted QV Diff")
```

If you exclude the three subjects with difference between the QValues from the two models are the predicted data more sensible?

```{r}
sub_stims_excluded = sub_stims %>%
  filter(!subnum %in% c("02","09", "14"))
subs = unique(sub_stims_excluded$subnum)
```


```{r}

oneIntPpc_exc = data.frame()
...
for(i in 1:length(subs)){
  ...
  oneIntPpc_exc = rbind(oneIntPpc_exc, sim_subj)
}

```

```{r}
sim_sanity_checks(oneIntPpc_exc, checks=c(1,3,4,5,6,7,8), compare_logits = T, compare_rts = T, true_data = sub_stims)
```
