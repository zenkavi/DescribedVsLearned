...
```
This model's way of accounting for the categorically faster choice for probFractalDraw = 1 trials is to bias the fractal integrator to start from the distortedQVDiff.

It doesn't seem to be doing a good enough job in speeding the decision. Is this because the distortedQVDiff is too small?

```{r}
sub_data %>%
  filter(probFractalDraw == 1) %>%
  mutate(rawQVDiff = QVLeft - QVRight) %>%
  ggplot(aes(distortedQVDiff, rawQVDiff))+
  geom_point()+
