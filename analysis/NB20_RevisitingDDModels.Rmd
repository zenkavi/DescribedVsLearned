...
The p of this choice happening at this timestep with my method: prob arb crossing for lott * prob lott suggesting left + prob arb crossing frac * prob frac suggesting left

```{r}
timeStep = 10
numTimeSteps = round(tmp$out$reactionTime*1000/timeStep)

mu_lott_rdv = tmp$out$dLott * numTimeSteps
s_lott_rdv = sqrt(tmp$out$sigmaLott^2*numTimeSteps)

mu_frac_rdv = tmp$out$dFrac * numTimeSteps
s_frac_rdv = sqrt(tmp$out$sigmaFrac^2*numTimeSteps)

mu_folded_diff = get_abs_diff_dist_moments(tmp$out$dLott, tmp$out$sigmaLott, tmp$out$dFrac, tmp$out$sigmaFrac)$diff_mu
mu_arb_rdv = mu_folded_diff * tmp$out$dArb * numTimeSteps
s_arb_rdv = sqrt(tmp$out$sigmaArb^2*numTimeSteps)

#p left
p_left = (1-pnorm(1, mu_arb_rdv, s_arb_rdv))* (1-pnorm(0, mu_lott_rdv, s_arb_rdv)) + pnorm(-1, mu_arb_rdv, s_arb_rdv)*(1-pnorm(0, mu_frac_rdv, s_frac_rdv))

#p right
p_right = (1-pnorm(1, mu_arb_rdv, s_arb_rdv))* (pnorm(0, mu_lott_rdv, s_arb_rdv)) + pnorm(-1, mu_arb_rdv, s_arb_rdv)*(pnorm(0, mu_frac_rdv, s_frac_rdv))

print(paste0("p(left) = ", round(p_left, 5)))
print(paste0("p(right) = ", round(p_right, 5)))
```

The # of time it happens at t = RT if you run 1000 particles
...
for(i in 1:n_particles){
  tmp_trial = sim_trial(dLott=0.03, dFrac=0.04, dArb=0.04, sigmaLott = 0.03, sigmaFrac = 0.03, distortedEVDiff = sub_data$distortedEVDiff[100], distortedQVDiff = sub_data$distortedQVDiff[100], probFractalDraw = sub_data$probFractalDraw[100], barrierDecay = 0, EVLeft = sub_data$EVLeft[100], EVRight = sub_data$EVRight[100], QVLeft = sub_data$QVLeft[100], QVRight = sub_data$QVRight[100])
  tmp_trial$iter = i
  sim_data = rbind(sim_data, tmp_trial)
}
```

```{r}
sim_data %>%
  mutate(numTimeSteps = round(reactionTime*1000/10)) %>%
  ggplot(aes(numTimeSteps))+
  geom_histogram(bins=25)+
  geom_hline(aes(yintercept = n_particles*p_left))+
  geom_vline(aes(xintercept = tmp$out$reactionTime*1000/10))
```
