...
Plot how many particles have crossed at certain time points
...
*Are the simulated particles distributed in the predicted manner?*
...
#   ungroup() %>%
#   group_by(time) %>%
#   mutate(num_remanining_part = sum(num_particles),
#          prop_particles = num_particles/num_remanining_part) %>%
...
demo_data = sim_data %>%
  ...
  mutate(state_bin=ifelse(is.na(state_bin), 22, state_bin)) %>%
  group_by(time, state_bin) %>%
  summarise(num_particles = n(),
