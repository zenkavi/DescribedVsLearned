...
Potential issue: trials where the attribute integrators move slowly in the same directions would result in unlikely slow responses.

```{r}
tmp = sim_trial(dLott=0.03, dFrac=0.03, dArb=0.07, sigmaLott = 0.003, sigmaFrac = 0.003, distortedEVDiff = .09, distortedQVDiff =.09, probFractalDraw =.5, barrierDecay = 0, EVLeft = NA, EVRight = NA, QVLeft = NA, QVRight = NA, debug=TRUE)
...
tmp$debug_df %>%
  ...
  theme(legend.position = "bottom",
        panel.grid = element_blank())+
  labs(color="", x = "Timestep (in 10 ms)", y="RDV")+
  annotate(geom="text", x=10, y = .6, label = annot_top, hjust=0)+
  annotate(geom="text", x=10, y = -.6, label = annot_bot, hjust=0)
```
...
Check if single trial fit function estimates the probabilities correctly on a very simpl example

```{r}
# i=1000
# tmp = sim_trial(dLott=0.03, dFrac=0.06, dArb=0.07, sigmaLott = 0.03, sigmaFrac = 0.03, distortedEVDiff = sub_data$distortedEVDiff[i], distortedQVDiff = sub_data$distortedQVDiff[i], probFractalDraw = sub_data$probFractalDraw[i], barrierDecay = 0, EVLeft = sub_data$EVLeft[i], EVRight = sub_data$EVRight[i], QVLeft = sub_data$QVLeft[i], QVRight = sub_data$QVRight[i], debug=TRUE)

tmp = sim_trial(dLott=0.03, dFrac=0.06, dArb=0.07, sigmaLott = 0.001, sigmaFrac = 0.001, distortedEVDiff = .5, distortedQVDiff = -.1, probFractalDraw = 2, barrierDecay = 0, EVLeft = NA, EVRight = NA, QVLeft = NA, QVRight = NA, debug=TRUE)
...
```{r}
tmp_fit = fit_trial(dLott=0.03, dFrac=0.06, dArb=0.07, sigmaLott = 0.001, sigmaFrac = 0.001, distortedEVDiff = .5, distortedQVDiff = -.1, probFractalDraw = 2, barrierDecay = 0, EVLeft = NA, EVRight = NA, QVLeft = NA, QVRight = NA,
...
sim_subj = sim_task(stimuli, model_name = "model3", dLott=0.03, dFrac=0.06, dArb=0.05, sigmaLott = 0.03, sigmaFrac = 0.03)
# sim_subj
```
...
for(i in 1:length(dLott_range)){
  for(j in 1:length(dFrac_range)){
    for(k in 1:length(dArb_range)){
      ...
      test_dFrac = dFrac_range[j]
      test_dArb = dArb_range[k]
      test_nll = get_task_nll(data_ = sim_subj, par = c(test_dLott, test_dFrac, test_dArb, sigma_fix, sigma_fix), par_names_ = c("dLott", "dFrac", "dArb","sigmaLott", "sigmaFrac"), model_name = "model3", fix_pars_ = list())
      recovery_df = rbind(recovery_df,
                          data.frame(test_dLott = test_dLott, test_dFrac = test_dFrac, test_dArb = test_dArb, nll = test_nll))
    }
  }
...
- Model all subjects together? Individual MLEs? Or can you get hierarchical Bayesian posteriors?
