...
Compute which state each particle is in based on the (continuous) RDV
...
Plot estimated pdf of each state for the summed RDV for the same time steps.
...
true_liks %>%
  ...
  geom_abline(aes(intercept=0, slope=1))
```
Ok so try it for a set of distorted EV differences (akin to a subject). Note this is integrating only on lotteries so not simulating/modeling the actual task yet.
...
Confirm that this sum of likelihoods for each trial would be lower if you used the incorrect drift rate and sigma.
...
Ok so try it with a model integrating on both attributes instead of just on lotteries.
