...
for(i in 1:nrow(est_data)){
  est_data$state_bin[i] = which.min(abs(states - est_data$states[i]))
}
```

*The simulated particles are distributed (above) in the predicted (below) manner**

```{r}
sim_data %>%
  group_by(time, state_bin) %>%
  summarise(num_particles = n(), 
            prop_particles = num_particles/n_particles,#once particles begin crossing would you need to normalize by the remaining particles?
            .groups='keep') %>%
  mutate(data_type="obs") %>%
  select(time, state_bin, prop_particles, data_type) %>%
  rbind(est_data %>%
          group_by(time) %>%
          mutate(norm_pdf_states = pdf_states/sum(pdf_states),
                 norm_pdf_states = ifelse(is.na(norm_pdf_states), 1, norm_pdf_states),
                 data_type="pred") %>%
          rename(prop_particles=norm_pdf_states) %>%
          select(time, state_bin, prop_particles, data_type))%>%
  # filter(time < 10) %>%
