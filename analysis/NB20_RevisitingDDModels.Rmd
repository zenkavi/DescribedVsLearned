...
```{r}
m4b_1 = sim_task(sub_data, model_name = "model4b", dLott=0.03, dFrac=0.06, dArb=0.06, sigmaLott = 0.03, sigmaFrac = 0.03, sigmaArb = 0.01)
```
...
fit_trial(dLott=0.03, dFrac=0.06, dArb=0.06, sigmaLott = 0.03, sigmaFrac = 0.03, sigmaArb = 0.01, barrierDecay=0, distortedEVDiff = m4b_1$distortedEVDiff[i],
...
```{r}
data.frame(var1 = rnorm(2500, mean = 3, sd = 1),
           var2 = rnorm(2500, mean = -1, sd = 1)) %>%
  ...
  geom_vline(aes(xintercept=ave))+
  # annotate(aes(label=as.character(round(ave,3))), "text", x=-4, y=200)+
  facet_grid(key~.)+
  scale_x_continuous(breaks = seq(-5, 6, .5))
```

```{r}
data.frame(var1 = rnorm(10000, mean = 1.529, sd = .438),
           var2 = rnorm(10000, mean = -2.167, sd = 3.793)) %>%
  mutate(avar1 = abs(var1),
         avar2 = abs(var2),
         var3 = avar1 - avar2) %>%
  gather(key, value) %>%
...
```{r}
get_abs_dist_moments(-2.167, 3.793)
```
