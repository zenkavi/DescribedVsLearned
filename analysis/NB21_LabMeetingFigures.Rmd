...
qv_theta_mod = clean_beh_data %>%
  ...
  unnest(tidied) %>%
  filter(term == "scale(fractal_qv_diff_theta)") %>%
  select(probFractalDraw, term, estimate, std.error)
```

```{r}
p = rbind(truev_mod, qv_mod, qv_theta_mod) %>%
  mutate(reg = ifelse(term == "scale(lottery_ev_diff)", "Lottery EV", ifelse(term == "scale(fractal_prob_diff)", "True Fractal P", ifelse(term == "scale(fractal_qv_diff)","Fractal QV", "Fractal QV * theta"))),
         theta = ifelse(term %in% c("scale(fractal_qv_diff_theta)", "scale(fractal_qv_diff)"), 1, 0),
  ...
  scale_color_manual(values = c(cbbPalette[3], cbbPalette[3:1]))+
...
p = rbind(truev_mod, qv_mod, qv_theta_mod) %>%
  mutate(reg = ifelse(term == "scale(lottery_ev_diff)", "Lottery EV", ifelse(term == "scale(fractal_prob_diff)", "True Fractal P", ifelse(term == "scale(fractal_qv_diff)","Fractal QV", "Fractal QV * theta"))),
         theta = ifelse(term %in% c("scale(fractal_qv_diff_theta)"), 1, 0),
         theta = as.factor(theta)) %>%
  ggplot(aes(probFractalDraw, estimate, col=reg, group=reg,alpha=theta))+
  geom_point()+
  geom_line()+
...
## RL implied QVs across different models
