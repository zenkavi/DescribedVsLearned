...
for (i in 1:length(pFrac_vals)){
  ...
  tmp = sim_trial(d = .03, sigma = .06, distortedEVDiff = distortedEVDiff, distortedQVDiff = distortedQVDiff, probFractalDraw = pFrac, barrierDecay = 0, bias = 0, EVLeft = NA, EVRight = NA, QVLeft = NA, QVRight = NA, debug=TRUE, stimDelay = stimDelay)
  ...
  demo_plots[[i]] = tmp$debug_df %>%
    ...
    theme(legend.position = "none",
          panel.grid = element_blank())+
    labs(color="", x = "Timestep (in 10 ms)", y="RDV")+
    annotate(geom="text", x=5, y = .6, label = annot_top, hjust=0, size=2)
  }
