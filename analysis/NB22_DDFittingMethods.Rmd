...
```{r}
get_task_nll(data_ = sim_data, par = c(.01, .01, .01, .01), par_names_ = c("dLott", "dFrac", "sigmaLott", "sigmaFrac"), model_name = "model2", fix_pars_ = list())
```
...
for(i in 1:length(d_range)){
  for(j in 1:length(sigma_range)){
    test_d = d_range[i]
    test_sigma = sigma_range[j]
    test_nll = get_task_nll(data_ = sim_data, par = c(test_d, test_sigma), par_names_ = c("dLott", "dFrac", "sigmaLott", "sigmaFrac"), model_name = "model2", fix_pars_ = list())
    recovery_df = rbind(recovery_df,
                        data.frame(test_d = test_d, test_sigma = test_sigma, nll = test_nll))
