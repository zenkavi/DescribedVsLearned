...
for(i in 1:length(unique(oneIntEstsAsymmLinear_earlyInt$subnum))){
  ...
  sim_subj = sim_task(cur_stims, model_name = "model1ea", d=cur_pars$d, sigma=cur_pars$sigma, theta=unique(cur_stims$theta))
  sim_subj$subnum = cur_sub
  oneIntPpcAsymmLinear_earlyInt = rbind(oneIntPpcAsymmLinear_earlyInt, sim_subj)
}
...
```{r}
sim_sanity_checks(oneIntPpcAsymmLinear_earlyInt, checks=c(1,3,4,5,6,8), compare_logits = T, compare_rts = T, true_data = sub_stims_asym)
```
