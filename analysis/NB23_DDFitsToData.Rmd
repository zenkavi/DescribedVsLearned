...
```{r}
tmp = oneIntPpcAsymmLinear_earlyInt[1:300,]
tmp$theta = thetas$theta[1]
...
# for(i in 1:nrow(tmp)){
#   fit_trial(d=cur_pars$d, sigma=cur_pars$sigma, choice = tmp$choice[i], reactionTime = tmp$reactionTime[i], probFractalDraw = tmp$probFractalDraw[i], EVLeft=tmp$EVLeft[i], EVRight=tmp$EVRight[i], QVLeft=tmp$QVLeft[i], QVRight=tmp$QVRight[i], distortedEVDiff = tmp$distortedEVDiff[i], distortedQVDiff = tmp$distortedQVDiff[i])
# }
