...
How do things look like for the prob distortion function used in the RL modeling?
...
data.frame(prob = seq(0,1,.1)) %>%
  mutate(distorted_prob_1a = (delta * (prob)^gamma) / ( (delta * (prob)^gamma) + (1-prob)^gamma ),
         distorted_prob_1b = exp(-delta*(-log(prob))^gamma)  ) %>%
