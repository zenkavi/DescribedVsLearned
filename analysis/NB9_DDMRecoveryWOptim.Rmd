...
- Likelihood surface depending on the delta and gamma - would a (big) grid search be the answer? See previous notebook.  
  - Change is larger for levels of gamma versus delta
...
- Moving to joint modeling of choice and RT with Stan by expanding the previous RL models? How well did these do in parameter recovery?
    - How does Dolan et al. do it?

```{r}

```

How much to perturb in what dimension? Stopping criteria for perturbations? I'd prefer not to write my own algorithm. Two potentially relevant papers:  
...
Write code to run many random starts and large grid searches on a cluster.

Parallelize random starts by generating a list of starter values and submitting each as a separate job.
Can also generate many random datasets.
How would you then organize the output of each?

What do you need:
```
Job submitting shell script looping through starter values
Batch script that calls the R script
R script that runs optim with the feeded starter values
```
