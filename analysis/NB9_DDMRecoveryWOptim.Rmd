...
test_trial_conditions = read.csv(paste0(helpers_path, 'ddModels/test_data/test_trial_conditions.csv'))
```
...
What are the true parameters for this dataset?

```{r}
true_pars = read.csv(paste0(helpers_path, 'ddModels/cluter_scripts/test_data/',data_name,'.csv'))
true_pars = true_pars %>%
...
For each of the 1000 iterations where did each parameter start and end? Where does the end point compare to the true parameters
...
Generate above plots for all random datasets

```{r}
ddm_par_recovery_report(model_ = "model1a", data_ = "sim_single_sub_data1", optim_out_path_=optim_out_path)
```
