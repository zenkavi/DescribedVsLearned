...
fit_task = function(data_, model_name_, pars_, fix_pars_ = list(), fit_trial_list_ = fit_trial_list, debug=FALSE){
  ...
  if("distortedEVDiff" %in% names(data_) == FALSE){
    print("distortedEVDiff not in data")
    data_$distortedEVDiff = NA
  }
  
  if("distortedQVDiff" %in% names(data_) == FALSE){
    print("distortedQVDiff not in data")
    data_$distortedQVDiff = NA
  }
