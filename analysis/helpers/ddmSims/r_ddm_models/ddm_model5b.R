sim_trial = function(dArb, dLott, dFrac, sigmaArb, sigmaLott, sigmaFrac, delta, gamma, barrierDecay, barrier=1, nonDecisionTime=0, lotteryBias=0.1, timeStep=10, maxIter=400, epsilon = 0.002, debug=FALSE,...){
  ...
  while (time<maxIter){
    ...
    if (elapsedNDT < nonDecIters){
      ...
      arbitrator_mu_mean = dArb * ((1-probFractalDraw) * abs(lotteryRDV) - (distortedProbFractalDraw) * abs(fractalRDV))
