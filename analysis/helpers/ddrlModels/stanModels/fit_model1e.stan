...
model {
  ...
  for(i in 1:num_subjs){
    ...
    for (t in 1:num_trials_for_subj) {
      ...
      w_pfractal= theta[i]*trial_pFrac[i, t];
      w_plottery = (1-theta[i])*(1-trial_pFrac[i, t]);
      
      // Asymmetric distortion: None of lottery EVs; only distorting QVs
      opt_val[1] = (w_plottery * ev_left[i, t]) + (w_pfractal * qv[1]); 
      opt_val[2] = (w_plottery * ev_right[i, t]) + (w_pfractal * qv[2]) ;
