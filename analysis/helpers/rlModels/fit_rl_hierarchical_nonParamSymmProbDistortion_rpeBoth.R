...
if(file.exists(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_nonParamSymmProbDistortion_rpeBoth.RDS'))){
  fit = readRDS(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_nonParamSymmProbDistortion_rpeBoth.RDS'))
  ...
  m = stan_model(paste0(helpers_path,'rlModels/stanModels/fit_rl_hierarchical_nonParamSymmProbDistortion_rpeBoth.stan'))
  
  fit = sampling(m, data=m_data)
  saveRDS(fit, paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_nonParamSymmProbDistortion_rpeBoth.RDS'))
}

## Organize output
out = organize_stan_output(fit, 
                           subj_par_names=c("alpha", "beta", "w0", "w1", "w2", "w3", "w4", "w5", "w6", "w7", "w8", "w9", "w10"),
                           group_par_names=c("g_alpha", "g_beta", "g_w0", "g_w1", "g_w2", "g_w3", "g_w4", "g_w5", "g_w6", "g_w7", "g_w8", "g_w9", "g_w10"))
