...
if(file.exists(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeChosenBundleFractal.RDS'))){
  fit = readRDS(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeChosenBundleFractal.RDS'))
  ...
  m = stan_model(paste0(helpers_path,'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeChosenBundleFractal.stan'))
  
  fit = sampling(m, data=m_data)
  saveRDS(fit, paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeChosenBundleFractal.RDS'))
}

## Organize output
out = organize_stan_output(fit, 
                           subj_par_names=c("alpha","theta", "beta"),
                           group_par_names=c("g_alpha","g_theta", "g_beta"))
