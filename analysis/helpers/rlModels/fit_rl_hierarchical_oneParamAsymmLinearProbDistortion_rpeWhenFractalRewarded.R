...
if(file.exists(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeWhenFractalRewarded.RDS'))){
  fit = readRDS(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeWhenFractalRewarded.RDS'))
  ...
  m = stan_model(paste0(helpers_path,'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeWhenFractalRewarded.stan'))
  
  fit = sampling(m, data=m_data)
  saveRDS(fit, paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmLinearProbDistortion_rpeWhenFractalRewarded.RDS'))
}
