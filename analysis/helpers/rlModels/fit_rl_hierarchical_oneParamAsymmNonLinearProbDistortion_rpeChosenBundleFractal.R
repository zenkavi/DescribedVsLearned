...
if(file.exists(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmNonLinearProbDistortion_rpeChosenBundleFractal.RDS'))){
  fit = readRDS(paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmNonLinearProbDistortion_rpeChosenBundleFractal.RDS'))
  ...
  pe_update_left = extract_var_for_stan(clean_beh_data, choiceLeft) #redundant; could use choices
  ...
  m = stan_model(paste0(helpers_path,'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmNonLinearProbDistortion_rpeChosenBundleFractal.stan'))
  
  fit = sampling(m, data=m_data)
  saveRDS(fit, paste0(helpers_path, 'rlModels/stanModels/fit_rl_hierarchical_oneParamAsymmNonLinearProbDistortion_rpeChosenBundleFractal.RDS'))
}

## Organize output
out = organize_stan_output(fit, 
                           subj_par_names=c("alpha", "delta", "beta"),
                           group_par_names=c("g_alpha", "g_delta", "g_beta"))
