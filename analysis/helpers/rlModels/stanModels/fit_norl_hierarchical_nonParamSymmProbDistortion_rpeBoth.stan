...
data {
  ...
  real fractal_probs_left[num_subjs, 300];
  real fractal_probs_right[num_subjs, 300];
...
model {
  ...
  for(i in 1:num_subjs){
    ...
    for (t in 1:num_trials_for_subj) {
      ...
      opt_val[1] = ((1-w_pi) * ev_left[i, t]) + (w_pi * fractal_probs_left[i, t]);
      opt_val[2] = ((1-w_pi) * ev_right[i, t]) + (w_pi * fractal_probs_right[i, t]) ;
