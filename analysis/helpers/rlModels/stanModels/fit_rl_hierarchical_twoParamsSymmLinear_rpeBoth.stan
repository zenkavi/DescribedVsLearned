...
parameters {
  ...
  real<lower=0, upper=20> g_w_int;
  real<lower=0, upper=20> g_w_slope;
  real<lower=0> g_beta;
  real<lower=0, upper=1> alpha[num_subjs];
  real<lower=0, upper=20> w_int[num_subjs];
  real<lower=0, upper=20> w_slope[num_subjs];
...
model {
  ...
  w_int ~ normal(g_w_int, 1);
  w_slope ~ normal(g_w_slope, 1);
  ...
  for(i in 1:num_subjs){
    ...
    for (t in 1:num_trials_for_subj) {
      ...
      w_pi = w_int[i]+w_slope[i]*trial_pFrac[i, t];
