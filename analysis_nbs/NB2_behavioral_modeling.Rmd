...
tmp %>%
  mutate(delta_gt_1 = ifelse(delta>=1, "delta >= 1", "delta < 0 "),
...
## Fit model for all subjects hierarchically
...
```{r}
par_ests = organize_stan_output(fit, c("alpha","gamma", "delta", "beta", "g_alpha","g_gamma", "g_delta", "g_beta"))
```

```{r}
names(data.frame(extract(fit, c("alpha","gamma", "delta", "beta", "g_alpha","g_gamma", "g_delta", "g_beta"))))
```