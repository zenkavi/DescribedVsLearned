...
```{r}
truePars=data.frame(alpha=.3, beta=4.6, gamma=.3, delta = 0.5)
numSims = 1
sim_out = identifiability_analysis(numSims= numSims, truePars = truePars)
```
...
Change color scale such that mean log likelihoods more than 2 SD's of all mean log likelihoods away are completely dark and we see variability only in the close likelihoods
...
tmp = sim_out %>%
  ...
  summarise(logLik_m = mean(logLik),
            logLik_sd = sd(logLik),
            .groups='keep') %>%
  mutate(logLik_m_bin = ifelse(abs(logLik_m)> 200, NA, logLik_m))
...
tmp %>%
  ...
  theme(legend.position = "bottom")+
  labs(title = paste0("Mean of log likelihood from ", numSims, " simulations per cell"), 
       x = "\u03b1",
       y = "\u03b2",
       fill="")+
  annotate("point", x=3, y=9)
```
