...
- Choose a grid for each parameter
  ...
  - beta 0-5 by 0.5 (11 values)
  - gamma 0-1.6 by 0.2 (9 values)
  - delta 0-1.6 by 0.2 (9 values)
  - total = 9803 simulations
  
Plot heatmaps of loglikelihood with alpha vs beta on axes for combinations of gamma and delta. This would be 256 plots!

First get a sense of trial variability

```{r}
pars = data.frame(alpha=.4, beta=1.2, delta=.7, gamma=.9)
numSims = 50
logLiks = sim_logLiks(numSims, pars)
data.frame(logLiks) %>%
  ggplot(aes(logLiks))+
