...
Simulate 10 subjects with the following true parameters

```{r}
truePars = data.frame(alpha=0.3, beta=4, gamma=0.4, delta=0.5)
modelName = 'fit_twoValSystemsWithRL_hierarchical'
id_par_ests = identifiability_analysis(truePars, modelName)
```
...
```{r}
tmp = do.call("rbind", replicate(numSims, truePars, simplify = FALSE))
tmp = tmp %>% mutate(subnum =1:n())
tmp = tmp %>%
  gather(par, value, -subnum)

id_par_ests %>%
  ggplot(aes(value))+
  geom_histogram(alpha=.5, bins = 30)+
  facet_grid(subnum~par, scale="free")+
  geom_vline(aes(xintercept = value), tmp, color=cbbPalette[5])+
  theme(axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),
        panel.grid = element_blank(),
        strip.text.x = element_text(size=14),
