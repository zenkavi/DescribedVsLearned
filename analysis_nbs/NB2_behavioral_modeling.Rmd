...
Plot proportion of choiceLeft as a function of probFractalDraw
...
pp_data %>%
  ...
  summarise(propLeft = mean(choiceLeft), .groups='keep') %>%
  arrange(probFractalDraw) %>%
  ggplot(aes(factor(probFractalDraw), propLeft))+
  geom_boxplot()+
  geom_point(data=tmp, aes(factor(probFractalDraw), propLeft), color="red")+
  facet_wrap(~subnum)+
