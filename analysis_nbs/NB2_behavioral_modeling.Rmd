...
```{r}
truePars=data.frame(alpha=.3, beta=2.1, gamma=.6, delta = 0.8)
numSims = 3
...
tmp = sim_out %>%
  ...
  summarise(logLik_m = mean(logLik),
            logLik_sd = sd(logLik),
            .groups='keep') %>%
  mutate(label=ifelse(alpha==true_pars$alpha & beta==true_pars$beta & delta==true_pars$delta & gamma == true_pars$gamma, "x", NA))
...
tmp %>%
  ...
  facet_grid(gamma~delta)+
  theme(legend.position = "bottom")+
  labs(title = paste0("SD of log likelihood from ", numSims, "simulations per cell"), 
       x = "\u03b1",
       y = "\u03b2",
       fill="")
```
