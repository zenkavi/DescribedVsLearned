...
Rule out it's not due to distortion of p(Frac)
...
tmp %>%
  mutate(w_slope_gt_1 = ifelse(w_slope>=1, "w_slope >= 1", "w_slope < 1 "),
         overest_fractal = ifelse( (w_int+w_slope*.5) > .5, "overestimate fractal", "underestimate fractal")) %>%
  ggplot(aes(pFrac, wpFrac, group=subnum))+
  geom_line(aes(linetype = w_slope_gt_1, color = overest_fractal), alpha=.5)+
  geom_abline(slope=1, intercept=0, linetype="dashed")+
  labs(x="p(Fractal)", y="w(pFractal)", title="Distortion of p(Fractal) with linear fit", color = element_blank(), linetype=element_blank())+
  scale_color_manual(values=c(cbbPalette[3], cbbPalette[7]))+
  theme(legend.position = "bottom")
```
