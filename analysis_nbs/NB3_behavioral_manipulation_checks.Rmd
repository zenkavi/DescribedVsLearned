...
Random effects analysis estimating a separate slope for each subject for both regressors. This is an under-powered analysis and I did not find any systematic relationship. 
...
clean_beh_data %>%
  ...
  scale_color_manual(values=c(cbbPalette[2], cbbPalette[1]))
```
Does how much one chooses the left bundle relate to how much the fractal QV of the left bundle is overestimated? This is bizarre. Why would subjects choose the left bundle less frequently the more they overestimate its fractal QV in conflict trials regardless of the relevance of the attribute?
...
clean_beh_data %>%
  ...
  facet_grid(conflictTrial~probFractalDraw)+
  labs(x = "Subjective - True left fractal value")
...
```{r}
clean_beh_data %>%
  ggplot(aes(as.factor(probFractalDraw), log(reactionTime), col=conflictTrial))+
  geom_boxplot()+
  theme(legend.position="bottom")+
  scale_color_manual(values=c(cbbPalette[1], cbbPalette[2]))+
  labs(x = "p(Fractal Draw)", fill="", y = "Log Reaction Time")
```
