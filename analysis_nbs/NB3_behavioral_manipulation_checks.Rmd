...
The confusing thing about this graph is how the effect of the Q-value difference seems to persist in no conflict trials regardless of pFrac. I initially expected the Q-value difference not to have an effect when it is not relevant pFrac < 0.5. But it seems to have as strong of an effect as the EV difference.
...
Function to run logit on subject of data and get slopes and standard
...
m1 = brm(choseRelevant ~ leftQVAdv + (1|subnum),
        data=dat %>% filter(conflictTrial=="conflict"), family=bernoulli(link="logit"), silent=2, refresh=0)

summary(m1)

m2 = brm(choseRelevant ~ leftEVAdv + (1|subnum),
        data=dat%>% filter(conflictTrial=="no conflict"), family=bernoulli(link="logit"),  silent=2, refresh=0)
