...
In no conflict trials  
- when reward depends more on lotteries  
- when reward depends more on fractals
...
```{r}
if(file.exists(paste0(helpers_path, 'logitOut_choseLeft.RDS'))){
  fit = readRDS(paste0(helpers_path, 'logitOut_choseLeft.RDS'))
} else {
  ...
  for(i in 1:length(probFractalDrawVals)){
    ...
    for(j in 1:length(conflictTrialTypes)){
      ...
      m = brm(choiceLeft ~ leftQVAdv + (1|subnum),
      ...
      out_choiceLeft = rbind(out, c(curProbFractalDraw, curConflictTrial, "leftQVAdv", est, u95, l95))
      
      m = brm(choiceLeft ~ leftEVAdv + (1|subnum),
              data=dat %>% filter(conflictTrial==curConflictTrial), family=bernoulli(link="logit"), silent=2, refresh=0)
      ...
      out_choiceLeft = rbind(out, c(curProbFractalDraw, curConflictTrial, "leftEVAdv", est, u95, l95))
    }
  }
...
for(i in 1:length(probFractalDrawVals)){
  ...
  for(j in 1:length(conflictTrialTypes)){
    ...
    m = brm(choseRelevant ~ leftQVAdv + (1|subnum),
            data=dat %>% filter(conflictTrial==curConflictTrial), family=bernoulli(link="logit"), silent=2, refresh=0)
    ...
    out_choseRelevant = rbind(out, c(curProbFractalDraw, curConflictTrial, "leftQVAdv", est, u95, l95))
    
     m = brm(choseRelevant ~ leftEVAdv + (1|subnum),
            data=dat %>% filter(conflictTrial==curConflictTrial), family=bernoulli(link="logit"), silent=2, refresh=0)
    ...
    out_choseRelevant = rbind(out, c(curProbFractalDraw, curConflictTrial, "leftEVAdv", est, u95, l95))
