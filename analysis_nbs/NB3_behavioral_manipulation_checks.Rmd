...
source(paste0(helpers_path,'fit_twoValSystemsWithRL_hierarchical_linearW.R'))
fit_linearW = fit
g_par_ests_linearW = g_par_ests
par_ests_linearW = par_ests
...
clean_beh_data_linearW = par_ests_linearW %>%
  group_by(subnum, par) %>%
...
clean_beh_data_linearW = clean_beh_data_linearW %>%
  group_by(subnum) %>%
  do(get_qvals(.)) %>%
  ungroup()
...
clean_beh_data_linearW = clean_beh_data_linearW %>%
  ...
  mutate(leftQVAdv = leftQValue - rightQValue,
         ...
         wpFrac = w_int + w_slope*probFractalDraw,
...
There can be various cognitive explanations for the choice patterns we observe. E.g.  

- Overvaluing of lotteries (what the prob weighting function above would suggest; how should that be related to choice since it is derived from it too)  
- Discounting of QVs (haven't thought of a way to test this)  
- Noisy representation of QVs (see below for attempts)  

**Are these mutually exclusive? How can we test for each?**

Does the variance (noisiness) of the QV estimates relate to how much a subject relies on the EV or QV difference?
Random effects analysis estimating a separate slope for each subject for both regressors collapsing both conflict types. This is an under-powered analysis and I did not find any systematic relationship. 

```{r}
if(file.exists(paste0(helpers_path, 'logitOut_choiceLeft_randomEffects_conflictCollapsed.RDS'))){
  out_choiceLeft = readRDS(paste0(helpers_path, 'logitOut_choiceLeft_randomEffects_conflictCollapsed.RDS'))
} else {
  
  probFractalDrawVals = unique(clean_beh_data$probFractalDraw)
  out_choiceLeft = data.frame(Estimate=NA, Est.Error=NA, Q2.5=NA, Q97.5=NA, subnum=NA, var=NA, probFractalDraw=NA)
  ...
  saveRDS(out_choiceLeft, paste0(helpers_path, 'logitOut_choiceLeft_randomEffects_conflictCollapsed.RDS'))
