...
tmp %>%
  ...
  geom_line(aes(group=subnum),stat='smooth',formula = 'y~x', method = "glm", method.args = list(family=binomial), fullrange=TRUE, se=FALSE, alpha=.2, size=1, color=cbbPalette[1])+
  geom_smooth(formula = 'y~x', method = "glm", method.args = list(family=binomial), fullrange=TRUE, color=cbbPalette[2])+
...
- when reward depends more on fractals people *incorrectly* choose the bundle with the better fractal and better lottery less frequently. Their decision correctly depends less on the lottery and more on the fractals but their inference about the fractal values are more noisy because it involves learning and cannot be read off the screen.
...
tmp %>% 
  ...
  labs(x="p(Fractal)",y="Proportion of better lottery choices")
...
```{r}
tmp %>%
  select(choiceLeft, leftQVAdv, leftEVAdv, leftbundleValAdv, probFractalDraw, conflictTrial) %>%
  gather(key, value, -choiceLeft, -probFractalDraw, -conflictTrial) %>%
  mutate(key = ifelse(key == "leftEVAdv", "EV(left)-EV(right)", ifelse(key == "leftQVAdv","QV(left)-QV(right)", "V(left) - V(right)"))) %>%
...
```{r}
if(file.exists(paste0(helpers_path, 'logitOut_choiceLeft_3DVs.RDS'))){
  fit = readRDS(paste0(helpers_path, 'logitOut_choiceLeft_3DVs.RDS'))
} else {
  ...
  for(i in 1:length(probFractalDrawVals)){
    ...
    for(j in 1:length(conflictTrialTypes)){
      ...
      m = brm(choiceLeft ~ leftQVAdv + leftEVAdv + leftbundleValAdv + (1|subnum),
      ...
      est = coef(m)$subnum[1,1,4]
      u95 = coef(m)$subnum[1,3,4]
      l95 = coef(m)$subnum[1,4,4]
      
      out_choiceLeft = rbind(out_choiceLeft, c(curProbFractalDraw, curConflictTrial, "leftbundleValAdv", est, u95, l95))
    }
  }
  ...
  saveRDS(fit, paste0(helpers_path, 'logitOut_choiceLeft_3DVs.RDS'))
}
```
