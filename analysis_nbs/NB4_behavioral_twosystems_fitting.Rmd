...
delta = 1.1
gamma = 1
w = (delta*pi^gamma)/(delta*pi^gamma + (1-pi)^gamma)
w
```
...
par_ests %>%
  ...
  labs(title = "Posterior distribution of probability distortion \U03B3",
       xlab="", ylab="")
```
...
par_ests %>%
  ...
  geom_histogram(alpha=.5, bins=50)+
  facet_wrap(~subnum, scales='free_y')+
  labs(title = "Posterior distribution of lottery bias \U03B4",
       xlab="", ylab="")
```
