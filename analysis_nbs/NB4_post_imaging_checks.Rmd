...
```
Models where the RPE is not updated on each trial tend to estimate higher learning rates.
...
How about for the relevant rpe regressor for the best fitting model?
Original -->  rpeLeftRightSum
rpeChosenBundleFractal --> ifelse(choiceLeft, leftFractalRpe, rightFractalRpe)
rpeWhenFractalRewarded --> ifelse(fractalDraw, rpeLeftRightSum, 0)

```{r}
names(clean_beh_data_original)
str(clean_beh_data_original$fractalDraw)
```
