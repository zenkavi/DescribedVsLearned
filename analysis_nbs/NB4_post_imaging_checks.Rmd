...
clean_beh_data %>%
  ...
  # ggplot(aes(leftBundleVal, rightBundleVal, col=alpha))+
  geom_point()+
  geom_abline(aes(intercept=0, slope = 1), linetype="dashed")+
  facet_wrap(~subnum)
```
How much the value of the bundles correlate with each other is associated with learning.
...
clean_beh_data %>%
  ...
  ggplot(aes(leftQValue, rightQValue, color=alpha))+
  geom_point()+
  facet_wrap(~lab_col)+
  geom_abline(aes(intercept=0, slope = 1), linetype="dashed")
```

Change in Q Values by learning rate

Do this accounting for the true value

```{r}
clean_beh_data %>%
  mutate(trueValDiffLeft = fractalLeftProb - leftQValue,
         trueValDiffRight = fractalRightProb - rightQValue,
         session=as.factor(session)) %>%
  select(trialNum, leftQValue, rightQValue, subnum, session) %>%
  gather(key, value, -trialNum, -subnum, -session) %>%
  filter(key == "rightQValue",
         subnum %in% c("02","07","19")) %>%
  ggplot(aes(trialNum, value))+
