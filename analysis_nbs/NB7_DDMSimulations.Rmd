...
```{r}
sim_trial = function(dArb, dAttr, sigmaArb, sigmaAttr, barrierDecay, barrier=1, nonDecisionTime=0, lotteryBias=0.1, timeStep=10, maxIter=400, epsilon = 0.002, stimDelay = 2000, debug=FALSE,...){
...
Make sure there is not integration for the irrelavant attribute for edge cases

```{r}
sim_trial_list[['model6']](dAttr=0.04, dArb=0.04, sigmaAttr = 0.03, sigmaArb = 0.03, EVLeft = .5, EVRight = 0.5, QVLeft = .5, QVRight = .5, probFractalDraw = 0, barrierDecay = 0)
```

## Check sim task

```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model6", dAttr=0.01, dArb=0.01, sigmaAttr = 0.03, sigmaArb = 0.03)
sim_sanity_checks(tmp)
```
