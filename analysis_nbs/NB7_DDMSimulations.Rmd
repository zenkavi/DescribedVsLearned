...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model2", d=0.045, sigma = 0.015)
sim_sanity_checks(tmp)
```
...
```{r}
with(tmp, table(timeOut, probFractalDraw))
```

Time outs by value difference

```{r}
tmp %>%
  mutate(QVDiff = abs(QVLeft - QVRight),
         EVDiff = abs(EVLeft - EVRight)) %>%
  select(EVDiff, QVDiff, timeOut, probFractalDraw) %>%
  gather(key, value, -timeOut, -probFractalDraw) %>%
  ggplot(aes(value, timeOut, color= key)) +
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model3", d=0.08, sigma = 0.01)
sim_sanity_checks(tmp)
```

```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model3", d=0.06, sigma = 0.008)
sim_sanity_checks(tmp)
```
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model4a", dLott=0.03, dFrac=0.04, dArb=0.04, sigmaLott = 0.03, sigmaFrac = 0.03, sigmaArb = 0.03)
sim_sanity_checks(tmp)
```

```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model4", dAttr=0.025, dArb=0.05, sigmaAttr = 0.03, sigmaArb = 0.01)
sim_sanity_checks(tmp)
```
...
```{r echo=FALSE, eval=FALSE}
tmp = sim_trial_list[['model5']](dArb = .04, dAttr=0.01, sigmaArb = .01, sigmaAttr = .03, barrierDecay = 0, EVLeft = .7, EVRight = 0.5, QVLeft = 0, QVRight = 0, probFractalDraw = 0, debug=TRUE, nonDecisionTime = 0)
tmp$out
```
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model5", dAttr=0.025, dArb=0.05, sigmaAttr = 0.03, sigmaArb = 0.01)
sim_sanity_checks(tmp)
```

```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model5", dAttr=0.05, dArb=0.025, sigmaAttr = 0.03, sigmaArb = 0.01)
sim_sanity_checks(tmp)
```
...
### How to visualize goodness of fit across >2 parameters?
...
- 3 integrators
- Arbitrator integrator starts from 0. Lottery bias is a multiplication of the lottery integrator drift rate
- If pFractalDraw == 1 fractal and arbitrato integrator starts early. Arbitrator moves only towards fractal.
...
Why is it so fast for pFractalDraw == 0?

```{r}
tmp = sim_trial_list[['model6']](dArb = .04, dAttr = .02, sigmaArb = .01, sigmaAttr = .03, EVLeft = .7, EVRight = .5, QVLeft = .5, QVRight = .7, probFractalDraw = 0, lotteryBias = .25, barrierDecay = 0, debug = TRUE)
tmp$out
```
...
Why isn't it slowest for pFractalDraw == 0.5?

```{r}
tmp = sim_trial_list[['model6']](dArb = .04, dAttr = .02, sigmaArb = .01, sigmaAttr = .03, EVLeft = .7, EVRight = .5, QVLeft = .5, QVRight = .7, barrierDecay = 0, probFractalDraw = 0.5, lotteryBias = .25, debug = TRUE)
tmp$out
```
```{r}
tmp$debug_df %>%
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model6", dAttr=0.02, dArb=0.04, sigmaAttr = 0.03, sigmaArb = 0.01, lotteryBias = .25)
sim_sanity_checks(tmp)
```

```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model6", dAttr=0.02, dArb=0.04, sigmaAttr = 0.03, sigmaArb = 0.01, lotteryBias = 0)
sim_sanity_checks(tmp)
```

What difference does the attribute integrator drift rate make?

```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model6", dAttr=0.04, dArb=0.02, sigmaAttr = 0.03, sigmaArb = 0.01, lotteryBias = 0)
sim_sanity_checks(tmp)
```

Why are the pFrac <.4 trials so fast (even when there is no lottery bias)? Is it due to pre stim decisions (shouldn't be unless there is a bug)? No it isn't.

```{r}
with(tmp, table(decPreStim, probFractalDraw))
```

# Model 7: Switching between integrators

```{r}
source(paste0(helpers_path, 'ddmSims/ddm_model7.R'))
sim_trial_list[['model7']] = sim_trial
```

```{r}
tmp = sim_trial_list[['model7']](dArb = .04, dAttr = .02, sigmaArb = .01, sigmaAttr = .03, EVLeft = .7, EVRight = .5, QVLeft = .5, QVRight = .7, probFractalDraw = .5, lotteryBias = .25, barrierDecay = 0, debug = TRUE)
tmp$out
```
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model7", dAttr=0.02, dArb=0.03, sigmaAttr = 0.03, sigmaArb = 0.01, lotteryBias = .15)
sim_sanity_checks(tmp)
```
