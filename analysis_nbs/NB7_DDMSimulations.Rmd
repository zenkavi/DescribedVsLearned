...
```{r}
sim_data_m5_1 = sim_task(stimuli=sub_data, d = .033, sigma = .03, barrierDecay = 0.007)
```
...
get_choice_sumsq = function(sub_data, sim_data){
  ...
  out = out1 %>%
    ...
    mutate(evsqdv = (EVDiff_sim - EVDiff_true)^2,
           qvsqdv = (QVDiff_sim - QVDiff_true)^2,
           sqdv = evsqdv + qvsqdv)
...
find_best_par_combo = function(sub_data, d_par_space = ..., sigma_par_space = seq(.01, .1, .01), heatmaps = FALSE){
  ...
  opt_rt_pars = out[rt_sumsq == min(out$rt_sumsq),]
  opt_choice_pars = out[choice_sumsq == min(out$choice_sumsq),]
  opt_avg_pars = out[avg_sumsq == min(out$avg_sumsq),]
  
  return(list(opt_rt_d = opt_rt_pars$d, opt_rt_sigma = opt_rt_pars$sigma, 
              opt_choice_d = opt_choice_pars$d, opt_choice_sigma = opt_choice_pars$sigma, 
              opt_avg_d = opt_avg_pars$d, opt_avg_d = opt_avg_pars$sigma))
}
```
