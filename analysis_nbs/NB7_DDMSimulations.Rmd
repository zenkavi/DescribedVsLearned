...
```{r}
sim_sub_data1 = sim_task(sub_data = sub_data, d = .05, sigma = .02, nonDecisionTime = 0)
```

### Sanity checks for simulated data.

```{r}
sim_sanity_checks = function(sim_data){
  #Check 1
...
The number of timeout trials is still too high

```{r}
sim_sanity_checks(sim_sub_data1)
```

## Model 2: Begin integration about Q values earlier
...
```{r}
sim_sub_data2 = sim_task(sub_data = sub_data, d = .05, sigma = .02, nonDecisionTime = 0)
```
...
```
## Model 3: Begin integration about Q values earlier if that is the only relevant option
...
```{r}
sim_sub_data3 = sim_task(sub_data = sub_data, d = .05, sigma = .02, nonDecisionTime = 0)
```

### Sanity checks

```{r}
sim_sanity_checks(sim_sub_data3)
```

Why do the GLMs for the simulated data have convergence issues? Note this isn't the case for actual data.
...
clean_beh_data %>%
  filter(subnum == "01" | subnum == "10" | subnum == "19") %>%
   nest(data = -probFractalDraw) %>% 
  mutate(
    fit = map(data, ~ glm(choiceLeft ~ leftEVAdv + leftQVAdv, data = .x, family=binomial(link="logit"))),
    tidied = map(fit, tidy)
  ) %>% 
  unnest(tidied) %>%
  filter(term != "(Intercept)")
...
Are EVDiff and QVDiff perfectly correlated in simulated data? That doesn't make sense. They are 

```{r}

```

Is there perfect separation because there isn't enough noise in the 