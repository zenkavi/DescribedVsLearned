...
```{r}
sim_sanity_checks = function(sim_data, checks = c(1,2,3,4)){
  #Check 1
  if (1 %in% checks){
...
```{r}
sim_sub_data1 = sim_task(sub_data = sub_data, d = .01, sigma = .1)
```
...
sim_trial = function(d, sigma,  barrier=1, nonDecisionTime=0, bias=0, timeStep=10, maxIter=1000, epsilon = 0.0002, ...){
  ...
  while (time<maxIter){
    ...
    if (time < stimDelayIters){
      mu_mean = d*probFractalDraw*(QVLeft - QVRight)
    } else{ 
      if (elapsedNDT < nonDecIters){
        mu_mean = 0
        elapsedNDT = elapsedNDT + 1
...
```{r}
sim_sub_data3 = sim_task(sub_data = sub_data, d = .01, sigma = .1)
```
