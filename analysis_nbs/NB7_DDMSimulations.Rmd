...
```{r}
sim_trial = function(d, sigma, barrierDecay, barrier=1, nonDecisionTime=0, lotteryBias=0, fractalBias = 0, timeStep=10, maxIter=400, epsilon = 0.002, debug=FALSE,...){
  ...
  while (time<maxIter){
    ...
    if (elapsedNDT < nonDecIters){
      ...
      arbitrator_mu_mean = abs(lotteryRDV) - abs(fractalRDV)
...
```{r}
sim_trial(d = .001, sigma = .01, barrierDecay = 0, EVLeft = 0.9, EVRight = .5, QVLeft = 0, QVRight = 0, probFractalDraw = 0, debug=TRUE, nonDecisionTime = 200)
```

```{r}
sim_data_m4_1 = sim_task(stimuli=sub_data, d = .002, sigma = .02, barrierDecay = 0.005)
```
