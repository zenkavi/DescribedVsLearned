  ...
  while (time<maxIter){
    ...
    if (elapsedNDT < nonDecIters){
      ...
      if (time < stimDelayIters){
        # Any iteration before after fractal prob presentation but before stim presentation happens only for the probfractaldraw == 1 trials
...
```{r}
t(replicate(20, sim_trial(d = .005, sigma = .05, barrierDecay = .005, EVLeft = 0.7, EVRight = .5, QVLeft = 0, QVRight = 0, probFractalDraw = 0.8, debug=FALSE), simplify = 'dataframe'))
```

```{r}
sim_data_m2_1 = sim_task(stimuli=sub_data, d = .005, sigma = .05, barrierDecay = .005)
```
