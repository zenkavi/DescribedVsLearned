...
```{r}
sim_sub_data1 = sim_task(sub_data = sub_data, d = .02, sigma = .05, nonDecisionTime = 300)
```
...
```{r}
sim_sub_data2 = sim_task(sub_data = sub_data, d = .02, sigma = .05, nonDecisionTime = 300)
```

```{r}
summary(sim_sub_data2)
```
Compare simulated data to true data
...
comp_sub_df %>%
  ...
  select(data_type, probFractalDraw, subnum, log_rt) %>%
