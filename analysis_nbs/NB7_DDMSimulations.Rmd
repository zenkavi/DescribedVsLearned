...
```{r}
m4a_opt_out = find_best_par_combo(sub_data, model_name = "model4a", 
                                 d_par_space = d_par_space, 
                                 sigma_par_space = sigma_par_space)
```

```{r}
summarise_opt_out(m4a_opt_out)
```
...
```{r}
sim_data_m4a = sim_w_best_combo(m4a_opt_out, "model4a")
```

```{r}
sim_sanity_checks(sim_data_m4a$sim_data_opt_rt)
```

```{r}
if(is.na(sim_data_m4a$sim_data_opt_choice)){
  print("Same data as above.")
} else {
  sim_sanity_checks(sim_data_m4a$sim_data_opt_choice)
}
...
tmp = sim_trial_list[['model5a']](dArb = .04, dLott=0.03, dFrac=0.02, sigmaArb = .01, sigmaLott = .03, sigmaFrac = .03, barrierDecay = 0, EVLeft = .7, EVRight = 0.5, QVLeft = 0, QVRight = 0, probFractalDraw = 0, debug=TRUE, nonDecisionTime = 0)
tmp$out
```
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model5a", dLott=0.025, dFrac=0.04, dArb=0.04, sigmaLott = 0.03, sigmaFrac = 0.03, sigmaArb = 0.01)
sim_sanity_checks(tmp)
```
...
```{r}
m5a_opt_out = find_best_par_combo(sub_data, model_name = "model5a", 
                                 d_par_space = d_par_space, 
                                 sigma_par_space = sigma_par_space)
```

```{r}
summarise_opt_out(m5a_opt_out)
```

```{r}
sim_data_m5 = sim_w_best_combo(m5a_opt_out, "model5a")
```

```{r}
sim_sanity_checks(sim_data_m5a$sim_data_opt_rt)
```
...
```{r}
if(is.na(sim_data_m5a$sim_data_opt_choice)){
  print("Same data as above.")
} else {
  sim_sanity_checks(sim_data_m5a$sim_data_opt_choice)
}
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model6a", dLott=0.02, dFrac=0.02, dArb=0.04, sigmaLott = 0.03, sigmaFrac = 0.03, sigmaArb = 0.01, lotteryBias = .25)
sim_sanity_checks(tmp)
```
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model7a", dLott=0.015, dFrac=0.05, dArb=0.03, sigmaLott = 0.03, sigmaFrac = 0.03, sigmaArb = 0.01, lotteryBias = .15)
sim_sanity_checks(tmp)
```
