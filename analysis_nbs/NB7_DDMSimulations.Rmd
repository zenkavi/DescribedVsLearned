...
sim_trial = function(d, sigma,  barrier=1, nonDecisionTime=0, bias=0, timeStep=10, maxIter=1000, epsilon = 0.0002,...){
  ...
  while (time<maxIter){
    ...
    if (RDV >= barrier | RDV <= -barrier){
      ...
      #if decision is reached before the stim screen sample rt from distribution with empirical mean and sd for probFractalDraw == 1 trials
      if (RT<(stimDelay/1000)){
        RT=rnorm(1, mean = 1, sd = 0.6) 
        if(RT < 0){ #make sure RT is positive
          RT = RT * (-1)
        }
      }
...
```{r}
sim_sub_data_m2_1 = sim_task(sub_data = sub_data, d = .005, sigma = .05)
```
