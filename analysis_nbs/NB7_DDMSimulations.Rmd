...
```{r eval=FALSE, echo=FALSE}
m2b = sim_task(sub_data, model_name = "model2b", d=0.03, sigma = 0.01, delta = 2, gamma = 2, barrierDecay = .007)
sim_sanity_checks(m2b, yrange_lim = 50)
```
- Why are the logit values so high?

In the simulations choice depends only on these value differences. The model fits too well, there is very low residual deviance.
...
```{r eval=FALSE, echo=FALSE}
tmp = sim_task(sub_data, model_name = "model2b", d=0.02, sigma = 0.005, delta = 2, gamma = 2, barrierDecay = 0.002, epsilon = 0.03)
sim_sanity_checks(tmp)
```

- What do the trials on the slow end of the bimodal distribution look like? Are they for particularly hard trials? Should we add something to an effect of "if options are too close to each other choose..." randomly? the first one?

```{r}
m2b %>%
  filter(reactionTime > 3)
```
