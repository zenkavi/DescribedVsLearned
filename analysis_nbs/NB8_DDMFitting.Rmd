...
Are likelihoods higher on average for a paramerter combination that is far from the true parameters? 
...
Why are the likelihoods for the true parameter combination not higher?

```{r}
out = fit_trial(d=.04, sigma=0.01, barrierDecay=0, delta=3, gamma=3, EVLeft = .7, EVRight = .2, QVLeft = .7, QVRight = .2, probFractalDraw = .4, choice = "left", reactionTime = .7, debug=TRUE)
names(out)
```
...
true_dat = read.csv('/Users/zeynepenkavi/Downloads/true_dat.csv')
true_dat
```
...
```{r}
fit2_data = fit_task(sub_data, model_name = "model2b", d=0.05, sigma = 0.01, delta = 2, gamma = 2, barrierDecay = 0.001, epsilon = 0, nonDecisionTime = 0)
```