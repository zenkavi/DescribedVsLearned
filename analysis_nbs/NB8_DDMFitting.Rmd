...
```
Do the false parameters generate sufficiently different trials? Yes. They are slower and more noisy.

```{r}
tmp2 = 100 %>% 
  rerun() %>%
  map_df(~data.frame(sim_trial(d=.001, sigma=0.1, barrierDecay=0, delta=3, gamma=3, EVLeft = .7, EVRight = .2, QVLeft = .7, QVRight = .2, probFractalDraw = .4))) 
...
```{r}
pars = list(d=0.04, sigma = 0.01, delta = 3, gamma = 3, barrierDecay = 0)
fit1_data = fit_task(tmp1, model_name = "model2b", pars_ = pars)
fit1_data = fit1_data %>% mutate(par_type="true")
```

```{r}
pars = list(d=0.002, sigma = 0.1, delta = 3, gamma = 3, barrierDecay = 0)
fit2_data = fit_task(tmp1, model_name = "model2b", pars_ = pars)
fit2_data = fit2_data %>% mutate(par_type="false")
```
...
```{r}
rbind(fit1_data, fit2_data) %>%
  ggplot(aes(fill=par_type, log(likelihood))) +
  geom_histogram(position="identity", alpha=.5, bins=30)
```
Is something weird with the simulated data? I didn't notice anything.
...
Is something weird with the fits? What do the likelihoods look like?

Why isn't there more variability in the likelihoods for either dataset? Probably because it's always the same trial.

Why is the probability to cross a bound so small for the true parameters, especially despite the larger drift rate? --> Step through ddm_model2b::fit_trial to debug

```{r}
length(unique(fit1_data$likelihood))
fit1_data
```

```{r}
length(unique(fit2_data$likelihood))
fit2_data
```

```{r}
rm(tmp1, tmp2, fit1_data, fit2_data)
```
