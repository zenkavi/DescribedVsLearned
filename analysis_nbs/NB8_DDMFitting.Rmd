...
Are the sum of likelihoods higher using the true parameter combination compared to false combinations?
...
```{r}
pars = setNames(as.list(c(.02, .007)), c("d", "sigma")) 
fit_task(true_dat, model_name = "model2b", pars_ = pars, debug=TRUE)
```

Is the task neg log likelihood smaller for the true parameters compared to wrong ones?
...
```{r}
get_task_nll(data=true_dat, par=c(10, 1, .0004), par_names=c("d", "sigma", "epsilon"), model_name="model2b")
```

This takes forever so it doesn't work. 
- Work on a simpler example unrelated to our models to make sure the optim function works ok
- Can the optim function work with the parallelization?
- Use a smaller dataset than true_dat?

```{r}
optim_out = optim(c(.01, .01), get_task_nll, data=true_dat, par_names = c("d", "sigma"), model_name="model2b")
```
